...
A comparação dos indicadores coletados no baseline e no snapshot pós-refatoração revela padrões distintos entre os três atributos avaliados. No acoplamento, os indicadores da camada legada permaneceram praticamente estáveis — todos os oito serviços seguem acoplados diretamente ao Firebase SDK e sete das trinta e quatro telas ainda importam o SDK sem mediação — enquanto os indicadores do núcleo arquitetural novo apresentaram transformação completa: quatro interfaces de porta e quatro adapters foram criados do zero, e os três use cases extraídos dependem exclusivamente de interfaces, sem nenhuma importação direta do Firebase. Na coesão, a redução de 351 linhas no diretório \texttt{app/services/} e a criação de sete novos artefatos especializados — três use cases e quatro entidades de domínio — evidenciam o início de um movimento de redistribuição de responsabilidades, ainda incompleto em relação aos God Objects remanescentes. Na testabilidade, a transformação foi a mais profunda em termos absolutos: o sistema passou de zero artefatos isoláveis para teste para um conjunto composto por três use cases com injeção de dependência, quatro entidades de domínio puras e quatro suites de teste automatizadas — partindo de uma base em que nenhum teste automatizado existia.
